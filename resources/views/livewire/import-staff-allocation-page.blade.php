...
<div>
    <flux:heading size="xl" class="mb-4">Import Staff Course Allocations</flux:heading>
    
    <div class="space-y-6">
        <form action="{{ route('import.staff-courses.upload') }}" method="post" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <label class="label mb-4">Spreadsheet</label>
            <flux:text class="mb-4 text-red-500">Please ensure all courses are uploaded to the database first.</flux:text>
