                    ...
                    @foreach ($students as $student)
                        <li>
                            <flux:text><a class="text-blue-500"
                                    href="{{ route('student.show', $student->id) }}">{{ $student->name }}</a></flux:text>
                        </li>
                    @endforeach
                </ul>
            </div>
        ...
        <div class="w-1/2">
            ...
            <ul class="list-disc">
                @foreach ($assessments as $assessment)
                    <li>
                        <flux:text><a class="text-blue-500"
                                href="{{ route('assessment.show', $assessment->id) }}">{{ $assessment->type }}</a> -
                            {{ $assessment->deadline }}</flux:text>
                    </li>
                @endforeach
            </ul>
        </div>
